...
# Part-c : Added reset button and undo button feature - by Kexian Guo
# Main Reviewer: Kiranpreet Kaur Dhillon, Prince
import pygame
import sys
import math
...
class Board:
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            ...
            self.turn += 1
            if choice[current_player] == 0:  # ONLY HUMAN CAN UNDO
                move_history.push((row, col, player))
            return True
        return False
...
move_history = Stack()  # Initializes the move history stack
...
def reset_game():
    global board, overflowing, overflow_boards, current_player, has_winner, grid_row, grid_col, choice
    board = Board(GRID_SIZE[1], GRID_SIZE[0], p1_sprites, p2_sprites)
    overflowing = False
    overflow_boards = Queue()
    current_player = 0
...
reset_button = Button(900, 200, 200, 50, "Reset", reset_game)
...
def undo_move():
    ...
    if choice[current_player] == 0 and not move_history.is_empty():
        row, col, player = move_history.pop()
        board.board[row][col] -= player
        current_player = (current_player + 1) % 2


undo_button = Button(900, 300, 200, 50, "Undo", undo_move)
...
while running:
    ...
    reset_button.draw(window, font)  # add reset button
    undo_button.draw(window, font)  # add undo button
    if not has_winner:
